...
if($args.Count -eq 2){
    ...
    $SlidesLocation = $args[0]
    $SlidesDuration = $args[1]

    Write-Host $SlidesLocation $SlideDuration
    ...
    $timer.add_tick({
        ...
        #Grab slide from our slides array based on the current slide index. Set our offsets so that the image will be centered vertically and horizontally.
        ...
        Write-Host $global:currentIndex $global:slides.Count

    })

    #This base64 string holds the bytes that make up the icon. Set our Window form to our icon.
    ...
    #Grab every image at the specified slides directory (first parameter), resize the image, and add it to our slides array
    $files = Get-ChildItem $SlidesLocation -Recurse -Include *.jpg,*.jpeg,*.png,*.gif -ErrorAction SilentlyContinue
    ...
    $fsw = New-Object System.IO.FileSystemWatcher($SlidesLocation, '*.*')
    ...
    #Set our timer interval (how fast our slides change) to be however long the user specified in the second parameter in seconds.
    $timer.Interval = [int]$SlidesDuration * 1000
    ...
    #Display our main window. Everything past this point is when the user closes the Window.
